/*
 * Copyright 2021 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package com.example.androiddevchallenge.repository

import com.example.androiddevchallenge.R
import com.example.androiddevchallenge.model.Puppy
import com.example.androiddevchallenge.model.Sex

val puppies: List<Puppy> = listOf(
    Puppy(
        0,
        "Pochi",
        3,
        Sex.Male,
        "Make it so, procedure! None of these galaxies will be lost in plasmas like procedures in flights",
        R.drawable.puppy1_thumbnaill,
        R.drawable.puppy1
    ),
    Puppy(
        1,
        "Tama",
        1,
        Sex.Female,
        "The proton is more alien now than tribble. interstellar and revolutionary brave.Where is the clear ship?",
        R.drawable.puppy2_thumbnail,
        R.drawable.puppy2
    ),
    Puppy(
        2,
        "Taro",
        0,
        Sex.Male,
        "Dry spinach fully, then mix with oyster sauce and serve freshly in bucket. Chicken breasts can be jumbled with smooth mackerel, also try blending the porridge with sweet chili sauce.",
        R.drawable.puppy3_thubnail,
        R.drawable.puppy3,
    ),
    Puppy(
        3,
        "Coco",
        1,
        Sex.Female,
        "Est varius gemna, cesaris. A falsis, diatria fatalis brabeuta. Cur planeta favere? Dexter gemna recte consumeres genetrix est. Zirbus superbus saga est. Superbus homo rare anhelares triticum est. Nunquam amor accola. A falsis, hippotoxota dexter visus. Nocere mechanice ducunt ad emeritis luba.",
        R.drawable.puppy4_thumbnail,
        R.drawable.puppy4
    ),
    Puppy(
        4,
        "Mugi",
        1,
        Sex.Male,
        "Bassus, albus castors mechanice dignus de germanus, clemens homo. Accolas sunt luras de noster abaculus. Extums sunt vortexs de rusticus mens. Calcarias peregrinatione! Velox brodiums ducunt ad deus. Seculas experimentum in grandis tolosa! Extums congregabo, tanquam varius mons. Palus de bassus hilotae, acquirere clabulare! Eheu. Hydras sunt bromiums de alter lanista. Brodium de dexter lixa, locus nutrix! Teres historia diligenter magicaes hippotoxota est. Sectams sunt lapsuss de magnum fermium. Humani generiss cadunt in tubinga! Grandis torquiss ducunt ad deus. Pol, mirabilis exsul! ",
        R.drawable.puppy5_thumbnail,
        R.drawable.puppy5
    ),
    Puppy(
        5,
        "Sora",
        1,
        Sex.Male,
        "Marshmellow juice soup is just not the same without black cardamon and old crusted bagels. One must follow the individual in order to emerge the explosion of the happiness of strange extend. Emptiness, milk and a superior closest mind. Going to the realm of politics doesn’t trap conclusion anymore than grasping creates important love. The source has harmony, but not everyone hurts it. ",
        R.drawable.puppy6_thumbnail,
        R.drawable.puppy6
    ),
    Puppy(
        6,
        "Marron",
        0,
        Sex.Female,
        "Season the tofu with cold za'atar, onion powder, green curry, and pepper making sure to cover all of it. To the tangy watermelon add doughnut, chicken lard, peppermint tea and whole cracker crumps. What’s the secret to salted and soaked rhubarb? Always use large black pepper. Place the lobster in a grinder, and mash up carefully shredded with large olive oil. To the gooey leek add seaweed, chicken, coconut milk and bloody peanut butter. Mash up a handfull peaces of pickles in nine teaspoons of triple sec.",
        R.drawable.puppy7_thumbnail,
        R.drawable.puppy7
    ),
    Puppy(7, "Momo", 2, Sex.Female, "Ships view from x-ray visions like strange particles. Crewmates wobble with disconnection! The hur'q is more dosi now than machine. virtual and impressively dead. Tribbles warp with sensor at the sub-light center! The girl is more moon now than queen. boldly and pedantically dead. When the crew warps for nowhere, all vogons love senior, conscious ferengis.", R.drawable.puppy8_thumbnail, R.drawable.puppy8)
)